...
2. **Embeddings singleton**: `multilingual-e5-base` loads at **import time**, not lazily (any script importing `app` pays the load cost immediately). Wrapped in `_E5PrefixedEmbeddings`, which prepends the `query: ` / `passage: ` prefixes that e5 models require — applied consistently at ingestion and query time; improves retrieval in ES, EN and cross-lingual EN→ES. (Gated on `"e5" in EMBEDDING_MODEL`.)
...
4. **Retrieval** (`get_context`): **similarity** search returning top-`RETRIEVER_K` (=10) chunks from the festival's Chroma collection. Completeness is handled in the *corpus*, not by diversifying retrieval: each topic whose answer would otherwise be scattered is consolidated into a single fiche that fits one chunk — the full per-day line-up (all four stages + bilingual headers; needs `CHUNK_SIZE`≥~2570, hence 3200), all arrival modes ("cómo llegar"), a focused international-arrival fiche and a focused climate fiche — so one top-ranked chunk answers fully. (MMR was tried and reverted: with consolidated chunks it only pulled in distractors, e.g. artist bios contradicting the official schedule.) Schedule and logistics fiches carry a query-shaped first line so they rank #1 for their natural question. For follow-up questions, the previous user turn (from `history`) is prepended to the retrieval query so the day/topic carries over (e.g. "¿y en el escenario X?" inherits "sábado"). The query is then passed through `augment_query`, which appends the current year when no explicit year is present. Queries that already name a year are left untouched.
...
- `festival_txts_big/` — an untracked, richer **warm_up-only** corpus (granular topic folders, plus `.csv`/`.jpg` assets and `.DS_Store` macOS junk). Its content is exposed as a single festival folder `festival_txts_big/warm_up/` (310 `.txt` across topic subfolders: `bandas_warmup`, `horarios_warmup`, `alojamientos_warmup`, `salida_laverdad_warmup_tema`, etc.). To use it, set `DATA_DIR=./festival_txts_big` and re-ingest `warm_up`; only `**/*.txt` is loaded (CSV/JPG ignored). **News exclusion**: `ingest_corpus` skips the globs in `constants.INGEST_EXCLUDE_GLOBS` (currently the press-news folders `salida_laverdad_warmup_tema/` and `noticias_depuradas/`), because those one-off weather forecasts and past-edition lineup rumors were contaminating stable-data answers (general climate, transport, official lineup). This is the **active corpus** in the local `.env`. Note: ~36 files under `warm_up/warm_up/` were originally Mac OS Roman encoded and have been converted to UTF-8; `ingest_corpus` also sets `autodetect_encoding=True` as a safety net.
