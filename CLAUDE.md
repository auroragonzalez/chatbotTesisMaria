...
4. **Retrieval** (`get_context`): similarity search returning top-k chunks from the festival's Chroma collection. The query is first passed through `augment_query`, which appends the current year when the user's question contains no explicit year — biasing retrieval toward the current edition, since the corpus mixes editions (e.g. 2024/2025 recap news alongside the 2026 schedule). Queries that already name a year are left untouched.
